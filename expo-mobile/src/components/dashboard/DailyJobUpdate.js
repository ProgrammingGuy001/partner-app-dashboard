...
const DailyJobUpdate = ({ jobId }) => {
  ...
  const historyLabel = loadingHistory
    ? "Loading history"
    : `${updates.length} past update${updates.length === 1 ? "" : "s"}`;
  ...
  return (
    ...
    >
      <TouchableOpacity
        ...
        accessibilityLabel="Toggle daily update form"
      ...
      {!expanded ? null : (
        <View className="mt-5">
          ...
          {/* Notes */}
          <View className="mb-4">
            <Text className="text-xs font-semibold text-muted-foreground uppercase mb-1.5">
              Notes (optional)
            </Text>
            <TextInput
              value={notes}
              ...
              numberOfLines={3}
              maxLength={2000}
              accessibilityLabel="Daily update notes"
