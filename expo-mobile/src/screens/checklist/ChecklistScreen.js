...
const ChecklistScreen = ({ navigation, route }) => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      Alert.alert("Export failed", err?.message || "Could not export this checklist.");
    } finally {
    ...
    <SafeAreaView className="flex-1 bg-background">
      ...
      >
        ...
        <View className="flex-row items-center gap-3 mb-6">
          ...
          {/* Every checklist exports, ISM or not — unlike the blank workbook download. */}
          <TouchableOpacity
            onPress={handleExport}
            disabled={isExporting}
            accessibilityRole="button"
            accessibilityLabel="Export checklist as PDF"
